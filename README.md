...
```bash
# 装好 lark-cli，并完成飞书 OAuth；clone 后安装/更新 skill
git clone https://github.com/Moinsky-sht/course-sync.git
cd course-sync
./install.sh
cp .env.example .env
# 编辑 .env，只填 COURSE_SYNC_SITE_SYNC_TOKEN 即可；URL 默认是 wlbycuc.cn 课程同步接口

ssh oldmac 'crontab -e'
# 加。该模式从 Base 导出并推送网站，不会重新写 Base：
0 3 * * * cd /home/ubuntu/course-sync && ./daily_sync.sh --export-only --require-site-sync >> /var/log/cs.log 2>&1
```
...
| `COURSE_SYNC_SITE_SYNC_URL` | `https://wlbycuc.cn/api/integrations/courses/sync` | 网站课程同步 API，可按需覆盖 |
...
如果配置了 `COURSE_SYNC_SITE_SYNC_TOKEN`，导出完成后会把 `courses.json`
推送到网站本地课程库。`COURSE_SYNC_SITE_SYNC_URL` 可省略，默认使用
`https://wlbycuc.cn/api/integrations/courses/sync`。推送失败只输出错误，
本地导出文件仍然保留。

定时任务建议带上 `--require-site-sync`，这样未配置网站 API Token 或推送失败时会退出非 0，便于 cron 监控发现问题。

### 只读同步到网站（推荐给另一台机器）

另一台机器不需要网站源码、数据库或本地后端服务。最小条件是：

1. 安装并登录 `lark-cli`，当前用户能读取目标飞书 Base。
2. 在 course-sync 目录创建 `.env`，写入 `COURSE_SYNC_SITE_SYNC_TOKEN`。
3. 执行 `./daily_sync.sh --export-only --require-site-sync`。

这个模式只读取飞书 Base 并调用网站 API，不写飞书 Base，不生成新的课程判断，也不会触发网站部署。
