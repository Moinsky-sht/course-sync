...
[2] lark-cli VC API (会议录制匹配 minute token + 参会峰值人数)
        ↓
[3] Claude 判断（人工或 LLM）
        ↓ 生成 judgments.json
...
| `fetch_speakers_via_browser.py` | 获取参会统计（lark-cli VC API，文件名保留用于兼容旧流程） |
...
1. **少数妙记可能匹配不到 VC 会议** — 例如标题被改过、会议不是当前 user 组织/参与、录制关系不可见；这些记录会保留 `participant_count: null`，判断时应更保守。
2. **VC 参会统计需要额外权限** — 当前 user 需要能读取会议记录、录制关系和 participants；否则会出现 401/403 或 `not_matched`。
